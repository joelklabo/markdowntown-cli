...
export default function AtlasConceptsPage() {
  const slugs = listAtlasGuideSlugs("concepts");

  return (
    <main className="py-mdt-2">
      <Stack gap={4}>
        <Stack gap={2}>
          <Heading level="h1">Concepts</Heading>
          <Text tone="muted">Guides under atlas/guides/concepts/.</Text>
        </Stack>

        {slugs.length === 0 ? (
          <Text tone="muted">No concept guides yet.</Text>
        ) : (
          <div className="grid gap-mdt-3">
            {slugs.map((slug) => (
              <Link
                key={slug}
                href={`/atlas/concepts/${slug}`}
                className={cn(
