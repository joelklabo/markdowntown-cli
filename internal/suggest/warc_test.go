...
func TestWARCWriter(t *testing.T) {
	...
	payload, err := os.ReadFile(filepath.Join("..", "..", "testdata", "suggest", "response.txt"))
	if err != nil {
...
func TestWARCWriterMissingType(t *testing.T) {
	writer := NewWARCWriter(&bytes.Buffer{})
	if _, err := writer.WriteRecord(WARCRecord{TargetURI: "https://example.com"}); err == nil {
		t.Fatalf("expected error for missing record type")
	}
}
