...
func TestFetcherRobotsMissingAllows(t *testing.T) {
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/robots.txt" {
	...
	if result.Skipped {
		t.Fatalf("expected robots missing to allow fetch")
	}
	if len(result.Warnings) == 0 {
		t.Fatalf("expected warning for missing robots")
	}
}

func TestFetcherRedirectRejectsHost(t *testing.T) {
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/robots.txt" {
			_, _ = w.Write([]byte("User-agent: *\nAllow: /\n"))
			return
		...
		if r.URL.Path == "/doc" {
			http.Redirect(w, r, "https://example.invalid/doc", http.StatusFound)
			return
		}
	}))
	t.Cleanup(server.Close)
	...
	_, err = fetcher.Fetch(context.Background(), FetchSource{ID: "src", URL: server.URL + "/doc"})
	if err == nil {
