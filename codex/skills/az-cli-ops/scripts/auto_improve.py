...
OPS_NOTES = os.path.join(REF_DIR, "az-ops-notes.md")
...
def suggestion_for(code, msg):
    ...
    if code == "authorizationfailed" or "authorizationfailed" in msg_lower:
        return "RBAC issue: check role assignments and active subscription (`az account show`)."
    if code == "resourcegroupnotfound" or "resource group" in msg_lower and "not found" in msg_lower:
        return "Resource group missing: verify name or set default group with `az configure -d group=<rg>`."
    if code in ("invalidtemplatedeployment", "deploymentfailed"):
        return "Deployment failed: use `az deployment group show` and `az deployment operation group list` to inspect errors."
    if code == "invalidresourcename" or "invalid resource name" in msg_lower:
        return "Naming rule violation: validate resource names (Key Vault secrets must be alphanumeric with hyphens)."
    if code == "conflict" or "already exists" in msg_lower:
        return "Resource conflict: check if the resource already exists and update instead of create."
    if "missing subscription registration" in msg_lower:
        return "Provider not registered: run `az provider register -n <Provider>` for the missing provider."
    return ""
...
def main():
    ...
    if not entries:
        with open(OUT_FILE, "w", encoding="utf-8") as f:
            ...
            f.write("No usage data yet. Use `scripts/azx` to run az commands so learnings accumulate.\n")
        return
