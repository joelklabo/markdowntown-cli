...
const client = new Client(
  { name: "klabo-post-error-inspect", version: "1.0.0" },
  { capabilities: {} }
);
...
async function run() {
  ...
  const inspectRes = await client.callTool({
    ...
    arguments: {
      function: `() => {
        const keys = Object.keys(window).filter((key) => key.toLowerCase().includes('error')).slice(0, 20);
        const nextData = window.__NEXT_DATA__ ?? null;
        const nextErrors = {
          __NEXT_DATA__: nextData && nextData.err ? nextData.err : null,
          __NEXT_ERROR__: window.__NEXT_ERROR__ ?? null,
          __next_error__: window.__next_error__ ?? null,
          __NEXT_HYDRATION_ERROR__: window.__NEXT_HYDRATION_ERROR__ ?? null,
        };
        const errorBoundary = document.querySelector('h1')?.textContent ?? null;
        return { keys, nextErrors, errorBoundary };
      }`,
    },
  });

  console.log(JSON.stringify(parseEvalJson(extractText(inspectRes)), null, 2));
}
