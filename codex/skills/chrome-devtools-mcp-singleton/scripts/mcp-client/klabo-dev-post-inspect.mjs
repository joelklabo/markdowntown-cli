...
const client = new Client(
  { name: "klabo-dev-post-inspect", version: "1.0.0" },
  { capabilities: {} }
);
  ...
  const inspectRes = await client.callTool({
    ...
    arguments: {
      function: `() => {
        ...
        const nextError = nextData && nextData.err ? nextData.err : null;
        const errorId = Array.from(document.querySelectorAll('p')).map(p => p.textContent).find(t => t && t.includes('Error ID')) || null;
        const h1 = document.querySelector('h1')?.textContent ?? null;
        return { h1, errorId, nextError, errorText: errorText.includes('Application error') ? errorText.slice(0, 800) : null };
      }`,
    },
  });
