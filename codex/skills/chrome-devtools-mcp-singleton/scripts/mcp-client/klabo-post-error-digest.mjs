...
const client = new Client(
  { name: "klabo-post-error-digest", version: "1.0.0" },
  { capabilities: {} }
);
...
async function run() {
  ...
  const errorIdRes = await client.callTool({
    name: evalTool,
    arguments: {
      function: "() => Array.from(document.querySelectorAll('p')).map(p => p.textContent).find(t => t && t.includes('Error ID')) || null",
    },
  });

  console.log(JSON.stringify(parseEvalJson(extractText(errorIdRes)), null, 2));
}
